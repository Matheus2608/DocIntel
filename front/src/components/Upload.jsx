...
export const Upload = ({ onUploadSuccess }) => {
    ...
    const backendApiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8080/api/chats';
    ...
    const handleFile = async (file) => {
        ...
        try {
            ...
            // Extrai o nome do arquivo sem extensão como título
            const title = file.name.replace(/\.[^/.]+$/, '');

            // Nota: O backend espera o title como @PathParam, mas o endpoint atual não tem
            // um path variable definido. Vamos usar como query param por enquanto.
            // Se o backend mudar para @Path("/{title}"), ajuste para:
            // const response = await fetch(`${backendApiUrl}/api/chats/${encodeURIComponent(title)}`, {
            ...
            // Adiciona o nome do arquivo aos dados do chat para exibição
